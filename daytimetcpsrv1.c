...
void detailsrv(int id,int sockfd)
{
     ...
     switch(id)
     {
        ...
        case 3:
				...
				printf("Before read\n");
                while((length=read(sockfd,bufftmp,MAXLINE))>0)
                {
                        if(fwrite(bufftmp, sizeof(char), length, fp) < length)
                        {
...
again:
						...
						{
							Writen(connfd,"server has already slept\n",strlen("server has already slept\n"));
							Close(connfd);
							exit(0);	
						}
                     }//child fork
